...
from Ali_Express.models.escim import Esmm_C, VAE, Esmm
from Ali_Express.models.escm2 import Escm2
from Ali_Express.models.dcmt import DCMT

from common.loss import linear_annealing, vae_loss
from Ali_Express.eval import eval_on_testset
  
def train_prior_model(config, latent_dim,
  ...
  cvr_loss_weight = config["cvr_f_loss_weight"]
  ...
  for epoch in range(num_epochs):
    ...
    for _, (categorical_fields, numerical_fields, labels) in enumerate(train_loader):
      ...
      categorical_fields = categorical_fields[click_idx]
      numerical_fields = numerical_fields[click_idx]
      ...
      # caluculate losses
      output = model(categorical_fields, numerical_fields, click, z_v)
      ...
      total_loss = ctr_loss + ctcvr_loss + cvr_loss_weight * cvr_loss + l2_reg * l2_reg_loss
    ...
    txt = '\n'.join([
      f'Epoch={epoch}:',
      f' Prior model loss={running_total_loss:.5f},',
    ])
    print(txt)
  
  return model

def train_vae(config, latent_dim,
              categorical_field_dims, numerical_num,
              embedding_size,
              train_loader, device):
  lr = config["lr"]
  ...
  vae = VAE(categorical_field_dims=categorical_field_dims, 
            numerical_num=numerical_num,
            embedding_size=embedding_size,
            latent_dim=latent_dim).to(device)
  optimizer_v = optim.Adam(vae.parameters(), lr=lr, weight_decay=wd)
  num_epochs = 8
  ...
  for epoch in range(num_epochs):
    ...
    for _, (categorical_fields, numerical_fields, labels) in enumerate(train_loader):
      ...
      batch, recon_batch, mu, logvar = vae(categorical_fields, numerical_fields, click, conversion)
...
def train(config, train_loader, train_clicked_loader, val_loader, test_loader, categorical_field_dims, numerical_num, device):
  
  embedding_size = config["embedding_size"]
  latent_dim = (len(categorical_field_dims)+1) * config["latent_dim_multiplier"]
  ...
  for trial in range(1, num_trials+1):
    ...
    for model in train_models:
      ...
      elif model in ["esmm", "multi-ips", "multi-dr", "escm2-ips", "escm2-dr", "dcmt"]:
...
def train_ours(
  ...
  prior_model = train_prior_model(config, latent_dim, categorical_field_dims, numerical_num, train_clicked_loader, device)
  vae = train_vae(config, latent_dim, categorical_field_dims, numerical_num, embedding_size, train_clicked_loader, device)
  ...
  for epoch in range(1, num_epochs+1):
    ...
    for _, (categorical_fields, numerical_fields, labels) in enumerate(train_loader):
      ...
      click_idx = (click==1).nonzero()[:, 0]
      categorical_fields_clicked = categorical_fields[click_idx]
      numerical_fields_clicked = numerical_fields[click_idx]
      click_clicked = click[click_idx]
      conversion_clicked = conversion[click_idx]
      
      # Extract mu, sigma
      x, _, _, _ = vae(categorical_fields_clicked, numerical_fields_clicked, click_clicked, conversion_clicked)
      ...
      categorical_emb = prior_model.embedding(categorical_fields)
      numerical_emb = prior_model.numerical_layer(numerical_fields).unsqueeze(1)
      emb = torch.cat([categorical_emb, numerical_emb], 1).view(-1, prior_model.embed_output_dim)  
      ...
      # Prediction
      cvr_cf = prior_model.cvr_model(emb, click_intv, z_v)
      ...
      categorical_fields_cf = categorical_fields[cf_conv_0_idx]
      numerical_fields_cf = numerical_fields[cf_conv_0_idx]
      ...
      if cf_conv_1_idx.numel() != 0:
        categorical_fields_cf = torch.cat([categorical_fields_cf, categorical_fields[cf_conv_1_idx]], dim=0)
        numerical_fields_cf = torch.cat([numerical_fields_cf, numerical_fields[cf_conv_1_idx]], dim=0)
      ...
      output_cf = model(categorical_fields_cf, numerical_fields_cf)
    ...
    # Test
    test_ctr_auc_value, test_cvr_auc_value, test_ctcvr_auc_value, test_ctr_prauc_value, test_cvr_prauc_value, test_ctcvr_prauc_value = eval_on_testset(model, device, test_loader)
    test_results_dict[epoch] = {
      'CTR_AUC': test_ctr_auc_value,
      'CVR_AUC': test_cvr_auc_value,
      'CTCVR_AUC': test_ctcvr_auc_value,
      
      'CTR_PRAUC': test_ctr_prauc_value,
      'CVR_PRAUC': test_cvr_prauc_value,
      'CTCVR_PRAUC': test_ctcvr_prauc_value,
    }
...
def train_baseline(
  ...
  for epoch in range(1, num_epochs+1):
    ...
    for _, (categorical_fields, numerical_fields, labels) in enumerate(train_loader):
      ...
      if model_name in ["esmm"]:
        total_loss = ctr_loss + ctcvr_loss + l2_reg * l2_reg_loss
      
      elif model_name in ["multi-ips", "escm2-ips"]:
        cvr_loss = weighted_loss_fn(pcvr, conversion)
        ctr_pred_clip = torch.clamp(pctr, min=0.05, max=1.0)
        cvr_loss = cvr_loss / ctr_pred_clip
        cvr_loss = torch.multiply(cvr_loss, click)
...
def valid(model_name, model, device, val_loader):
  ...
  model.eval()
  for i, (categorical_fields, numerical_fields, labels) in enumerate(val_loader):
    categorical_fields, numerical_fields, labels = categorical_fields.to(device), numerical_fields.to(device), labels.to(device)
    click, conversion = labels[:, 0], labels[:, 1]
    ...
    del categorical_fields, numerical_fields, labels, click, conversion, output, pctr, pcvr, pctcvr
