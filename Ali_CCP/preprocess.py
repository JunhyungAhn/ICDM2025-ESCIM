...
from Ali_CCP.constants import common_feat_path, data_path, enum_path, use_columns, write_path
...
class Process(object):
    ...
    Link: https://github.com/xidongbo/AITM/blob/main/process_public_dataset.py
    '''
    def __init__(self):
        super().__init__()
    ...
    def filter_clicked_data(self):
        """
        Read the .train file, filter for rows where click == 1, and save to a new file.
        
        Args:
            train_file (str): Path to the .train file.
            output_file (str): Path to save the filtered file containing only clicked data.
        """
        c = 0
        with open(write_path + '.train', 'r') as fr:
            with open(write_path + '.train_clicked', 'w') as fw:
                header = fr.readline()  # Read the header
                fw.write(header)  # Write the header to the new file
                
                for line in fr:
                    line_list = line.strip().split(',')
                    
                    # Filter rows where 'click' (first column) is 1
                    if line_list[0] == '1':  
                        fw.write(line)  # Write only clicked rows to the new file
                        c += 1
                        if c % 100000 == 0:
                            print(f'{c} clicked rows written')
        
        print(f'Total clicked rows written: {c}')
